...
let stoneSellPriceDisplay;
let stoneQuantityDisplay;
let copperSellPriceDisplay;
let copperQuantityDisplay;
let ironSellPriceDisplay;
let ironQuantityDisplay;
let quartzSellPriceDisplay;
let quartzQuantityDisplay;
...
function healthBarHandler(item) {
    ...
    if(healthPercentage <= 0) {
        generateRewards(item);
        let currentItem = item;
        updateScreenValues(currentItem)
...
function updateScreenValues(currentItem) {
    if(items[currentItem].itemName === 'Stone'){
        stoneQuantityDisplay = document.querySelector('#stone-quantity')
        stoneQuantityDisplay.innerHTML = items[currentItem].quantity
        stoneSellPriceDisplay = document.querySelector('#stone-sell-value')
        stoneSellPriceDisplay.innerHTML = items[currentItem].sellValue
    }
    if(items[currentItem].itemName === 'Raw Copper'){
        copperQuantityDisplay = document.querySelector('#copper-quantity')
        copperQuantityDisplay.innerHTML = items[currentItem].quantity
        copperSellPriceDisplay = document.querySelector('#copper-sell-value')
        copperSellPriceDisplay.innerHTML = items[currentItem].sellValue
    }
    if(items[currentItem].itemName === 'Raw Iron'){
        ironQuantityDisplay = document.querySelector('#iron-quantity')
        ironQuantityDisplay.innerHTML = items[currentItem].quantity
        ironSellPriceDisplay = document.querySelector('#iron-sell-value')
        ironSellPriceDisplay.innerHTML = items[currentItem].sellValue
    }
    if(items[currentItem].itemName === 'Quartz'){
        quartzQuantityDisplay = document.querySelector('#quartz-quantity')
        quartzQuantityDisplay.innerHTML = items[currentItem].quantity
        quartzSellPriceDisplay = document.querySelector('#quartz-sell-value')
        quartzSellPriceDisplay.innerHTML = items[currentItem].sellValue
    }
}
...
function generateRewards(item){
    if(items[item].quantity === 0) {
        ++items[item].quantity;
        if (items[item]) {
            let inventoryElement = document.createElement('div')
            inventoryElement.innerHTML = `
            <div class="inventory-item" id="${items[item].itemName}">
            <div class="item-icon">
                <img src="${items[item].rewards.source}" alt="${items[item].rewards.alt}">
            </div>
            <div class="info-wrapper">
                <div class="iw-left-section">
                    <div class="name">${items[item].itemName}</div>
                    <div class="quantity" id="${items[item].quantityValueId}">
                        ${items[item].quantity}
                    </div>
                </div>
                <div class="iw-right-section">
                    <div class="price">
                        <span id="${items[item].sellValueId}">
                        ${items[item].sellValue}
                        </span>
                        <div>
            ...
            `
            inventoryPanelContentElement.appendChild(inventoryElement)
        }
...
function customRandomizer() {
    ...
    const range0 = 0.25; // 25% chance
    const range1 = range0 + 0.25; // 25% chance
    const range2 = range1 + 0.25; // 25% chance
